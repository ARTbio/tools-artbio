...
class HitContainer:
    ...
    def __init__(self, header):
        ...
        self.aligned_reads_start = dict()
        self.aligned_reads_stop = dict()

    def add_read(self, offset, polarity, size):
        """
        Add 1 to the number of reads starting at the same offset and ending in the same position
        """
        if polarity == "forward":
            if self.aligned_reads_start.has_key(offset) :
                self.aligned_reads_start[offset] += 1
            else :
                self.aligned_reads_start[offset] = 1
            if self.aligned_reads_stop.has_key(offset + size) :
    ...
    def count_reads(self, upstream_coord, downstream_coord, polarity):
        ...
        if polarity == "forward":
            ...
            for offset in xrange(upstream_coord, downstream_coord + 1):
                if self.aligned_reads_start.has_key(offset):
                    for read in self.aligned_reads_start[offset]:
                        overlaping += 1
        else:
            """
            If the polarity is - then check for each position
                                ...
                                if options.lattice:
                                    ...
                                    for position in positions:
                                        ...
                                        if hit_store[chrom].aligned_reads_stop.has_key(position):
                                            read_count -= hit_store[chrom].aligned_reads_stop[position]
                                        coverage[position] = read_count
                            else:
