...
if (args$first_plot_method == "Counts") {
...
## function parameters
par.settings.firstplot = list(layout.heights=list(top.padding=-1, bottom.padding = -3), plot.margin=unit(c(-0.5,1,1,1), "cm"))
par.settings.secondplot=list(layout.heights=list(top.padding=-1, bottom.padding = -3), strip.background=list(col=c("deepskyblue","lavender")) )
...
if (n_genes > 5) {page_height_simple = 16; page_height_combi=16; rows_per_page=10} else { # 11.69
...
for (i in seq(1,n_genes,rows_per_page/2)) {
    ...
    args_list=c(plot.list, list(nrow=rows_per_page+1, ncol=1, heights=rep(10,11),
                                ...
                                sub=textGrob(paste(bottom_first_method[[args$first_plot_method]], "/", bottom_extra_method[[args$extra_plot_method]]), gp=gpar(cex=1), just="bottom")
                                )
               )
